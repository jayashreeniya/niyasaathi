...
Remember: You are Niyasaathi, not Niva. Use this name consistently.`;
  ...
  _determineNextScriptItem(userData, userMessage) {
    ...
    if (currentItem.id === 'explore_causes_intro') {
      ...
      // Social anxiety
      const socialAnxietyKeywords = ['social anxiety', 'social fear', 'afraid', 'scared', 'anxious', '3', 'three', 'third'];
      ...
      // Work stress
      const workStressKeywords = ['work stress', 'job stress', 'career', 'work', 'job', '4', 'four', 'fourth'];
      if (workStressKeywords.some(keyword => userResponseLower.includes(keyword))) {
        userData.branch = 'work_stress';
        // Find the work stress followup intro
        for (let i = 0; i < this.lonelinessScript.length; i++) {
          const item = this.lonelinessScript[i];
          if (item.id === 'work_stress_followup_intro') {
            userData.script_progress = i;
            return `script_${i}`;
          }
        }
      ...
      // Health issues
      const healthKeywords = ['health', 'illness', 'medical', 'sick', '5', 'five', 'fifth'];
      if (healthKeywords.some(keyword => userResponseLower.includes(keyword))) {
        userData.branch = 'health_issues';
        // Find the health issues followup intro
        for (let i = 0; i < this.lonelinessScript.length; i++) {
          const item = this.lonelinessScript[i];
          if (item.id === 'health_issues_followup_intro') {
            userData.script_progress = i;
            return `script_${i}`;
          }
        }
